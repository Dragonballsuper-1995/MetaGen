...
def _llm_call(
    ...
    """Single non-streaming LLM call → parsed JSON dict."""
    settings = _get_settings()
    llm = get_llm()
    ...
    logger.info("LLM call completed in %.2fs (max_tokens=%d)", elapsed, max_tokens)
    _log_gpu_status()
...
def _llm_stream(
    ...
    """Streaming LLM call — yields individual token strings."""
    settings = _get_settings()
    llm = get_llm()
